...
class RecursiveDecomposer(dspy.Module):
    def __init__(self, cot: bool = True, schema_repr: SchemaRepresentation = SchemaRepresentation.M_SCHEMA):
        if cot:
            self.model = dspy.ChainOfThought(Decompose)
        else:
    ...
    def forward(self, db_schema: DBSchema, question: str) -> QPLQDTree:
        def rec(question: str, _parent: QPLQDTree | None = None) -> QPLQDTree:
            # Predict toplevel operator and sub questions
            output = self.model(
                db_schema=db_schema.ddl() if self.schema_repr == SchemaRepresentation.DDL else db_schema.m_schema(), 
                question=question
            )

            # Create tree without sub questions
            tree = QPLQDTree(
                question=question,
                db_id=db_schema.db_id,
                op=output.operator,
                decomposition_cot=output.reasoning if self.cot else None,
                parent=_parent
            )

            # Create child trees and link back to parent
            children = []
            for sq in output.sub_questions:
                children.append(rec(question=sq, _parent=tree))
            tree.children = tuple(children)

            return tree
        
        # Obtain decomposition tree and index it
        tree = rec(question)
...
class RecursiveCompleter(dspy.Module):
    def __init__(self, cot: bool = True, schema_repr: SchemaRepresentation = SchemaRepresentation.M_SCHEMA):
        if cot:
            self.model = dspy.ChainOfThought(Complete)
        else:
    ...
    def forward(self, tree: QPLQDTree, db_schema: DBSchema) -> None:
        def rec(tree: QPLQDTree) -> None:
            # Complete QPL for children
            for child in tree.children:
                rec(child)

            # Obtain prefix QPL based on children
            prefix_qpl = "\n".join([child.qpl for child in tree.children])
            
            # Add partial line to complete
            if tree.op == Operator.SCAN:
                children_str = "Table"
            else:
                children_str = f"[ {', '.join(f'#{child.line_num}' for child in tree.children)} ]"
            prefix_qpl += f"#{tree.line_num} = {tree.op} {children_str} ..."

            # Complete the QPL line
            output = self.model(
                db_schema=db_schema.ddl() if self.schema_repr == SchemaRepresentation.DDL else db_schema.m_schema(),
                question=tree.question,
                prefix_qpl=prefix_qpl
            )
            tree.qpl_line = output.last_line
...
class TextToQPL(dspy.Module):
    ...
    def forward(self, db_id: str, question: str) -> QPLQDTree:
        ...
        decomposition_tree = self.decomposer(db_schema=db_schema, question=question)
        self.completer(tree=decomposition_tree, db_schema=db_schema)
        return decomposition_tree
    

if __name__ == "__main__":
    dspy.configure(lm=dspy.LM("ollama_chat/gpt-oss"))
    text_to_qpl = TextToQPL()
    tree = text_to_qpl(db_id="concert_singer", question="Show name, country, age for all singers ordered by age from the oldest to the youngest.")
